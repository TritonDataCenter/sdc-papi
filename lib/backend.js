...
function (cb) {
    ...
    backend.once('connect', function () {
        ...
        backend.on('error', function (err) {
            log.warn({ err: err }, 'Moray: unexpected error occurred');
        });
...
function (params, meta, cb) {
    ...
    backend.getObject(bucket, uuid, function (err, obj) {
        if (err && err.name === 'ObjectNotFoundError') {
            return backend.putObject(bucket, uuid, pkg, meta, function (er1) {
                    if (er1) {
                        return cb(er1);
                    }
                    return cb();
            ...
            var error = err || 'ObjectAlreadyExistsError';
            return cb(error);
        }
    });
};
...
function (uuid, meta, cb) {
    ...
    return self.backend.getObject(self.bucket, uuid, meta, function (err, obj) {
        ...
        var pkg = decode(obj.value, self.schema);
        return cb(null, pkg);
    });
};
...
function (uuid, params, meta, cb) {
    ...
    self.backend.putObject(self.bucket, params.uuid, pkg, meta, function (err) {
        if (err) {
            return cb(err);
        }
        return cb();
...
function (uuid, meta, cb) {
    ...
    self.backend.delObject(self.bucket, uuid, meta, function (err) {
        cb(err);
    });
};
