    ...
    if (req.params.filter) {
        ...
        keys.forEach(function (p) {
            if (req.params[p]) {
                ...
                // or none at all
                if (p === 'owner_uuid') {
                    pieces.push(util.format('(|(%s=%s)(!(owner_uuids=*)))', k,
                                            req.params[p]));
                } else {
...
function getPkg(req, res, next) {
    ...
    if (req.params.owner_uuid && req.pkg.owner_uuids &&
        req.pkg.owner_uuids.indexOf(req.params.owner_uuid) === -1) {
        return next(new restify.ResourceNotFoundError('Package ' +
                req.params.uuid + ' does not exist'));
    }
...
function updatePkg(req, res, next) {
    ...
    var immutable = Object.keys(schema).filter(function (key) {
    ...
    if (!req.params.skip_validation) {
        var errs = validations.validate(req.pkg, req.config.schema);
